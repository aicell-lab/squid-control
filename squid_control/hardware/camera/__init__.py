...
def get_camera(camera_type, focus_camera_type=None):
    ...
    if camera_type == "Toupcam":
        try:
            import squid_control.hardware.camera.camera_toupcam as camera
        except ImportError:
            print("Problem importing Toupcam, defaulting to default camera")
            import squid_control.hardware.camera.camera_default as camera
    elif camera_type == "FLIR":
        try:
            import squid_control.hardware.camera.camera_flir as camera
        except ImportError:
    ...
    if focus_camera_type == "Toupcam":
        try:
            import squid_control.hardware.camera.camera_toupcam as camera_fc
        except ImportError:
            print("Problem importing Toupcam for focus, defaulting to default camera")
            import squid_control.hardware.camera.camera_default as camera_fc
    elif focus_camera_type == "FLIR":
        try:
            import squid_control.hardware.camera.camera_flir as camera_fc
        except ImportError:
