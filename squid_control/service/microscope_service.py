...
from squid_control.service.video_stream import MicroscopeVideoTrack
...
class MicroscopeHyphaService:
    def __init__(self, is_simulation, is_local, config_name=None):  # noqa: PLR0915
        ...
        self.squidController = SquidController(
            is_simulation=is_simulation, config_name=config_name
        )
        ...
        self.snapshot_manager = (
            None  # Will be initialized after artifact_manager in setup()
        )
        self.server_url = (
            "http://192.168.2.1:9527" if is_local else "https://hypha.aicell.io/"
        )
        self.server = None
        ...
        if base_service_id and is_simulation and config_name:
            ...
            if config_name != "HCS_v2":  # Only append if not default config
                self.service_id = (
                    f"{base_service_id}-{config_name.lower().replace('hcs_v2_', '')}"
                )
        ...
        self.simulation_zarr_url = None
        self.active_simulation_sample = None
        self._sync_active_simulation_sample()
        ...
        self.scanning_in_progress = (
            False  # Flag to prevent video buffering during scans
        )
    ...
    def load_authorized_emails(self):
        ...
        if not authorized_users:
            logger.info(
                "No AUTHORIZED_USERS environment variable set - allowing all authenticated users"
            )
            return None
        ...
        try:
            ...
            if isinstance(authorized_users, str):
                ...
                if "," in authorized_users:
                    authorized_emails = [
                        email.strip()
                        for email in authorized_users.split(",")
                        if email.strip()
                    ]
                else:
                    # Single email without comma
                    authorized_emails = (
                        [authorized_users.strip()] if authorized_users.strip() else []
                    )
            else:
            ...
            if not isinstance(authorized_emails, list):
                logger.warning(
                    "AUTHORIZED_USERS must be a list of emails - allowing all authenticated users"
                )
                return None
            ...
            if valid_emails:
                logger.info(
                    f"Loaded {len(valid_emails)} authorized emails from AUTHORIZED_USERS"
                )
                return valid_emails
            else:
                logger.warning(
                    "No valid emails found in AUTHORIZED_USERS - allowing all authenticated users"
                )
                return None

        except Exception as e:
            logger.error(
                f"Error parsing AUTHORIZED_USERS environment variable: {e} - allowing all authenticated users"
            )
            return None

    def _build_app_url(self, server, app_id: str) -> str:
        return f"{self.server_url.rstrip('/')}/{server.config.workspace}/apps/{app_id}"

    def _get_simulation_zarr_service_id(self) -> Optional[str]:
        if not self.is_simulation or not self.service_id:
            return None
        return f"{self.service_id}-zarr"

    def _infer_simulation_sample_key(
        self,
        zarr_dataset_path: Optional[str] = None,
        config_name: Optional[str] = None,
    ) -> Optional[str]:
        if zarr_dataset_path:
            for sample_name, sample_info in SIMULATION_SAMPLES.items():
                if sample_info.get("zarr_dataset_path") == zarr_dataset_path:
                    return sample_name

        if config_name:
            normalized_config = config_name.upper().replace("-", "_").replace(" ", "_")
            alias_key = _SAMPLE_ALIASES.get(normalized_config, normalized_config)
            if alias_key in SIMULATION_SAMPLES:
                return alias_key

            for sample_name, sample_info in SIMULATION_SAMPLES.items():
                if sample_info.get("config_name") == config_name:
                    return sample_name

        return None

    def _sync_active_simulation_sample(
        self, sample_key: Optional[str] = None
    ) -> Optional[str]:
        if not self.is_simulation:
            self.active_simulation_sample = None
            return None

        inferred_sample = sample_key or self._infer_simulation_sample_key(
            zarr_dataset_path=getattr(
                getattr(self.squidController, "camera", None),
                "ZARR_DATASET_PATH",
                None,
            ),
        ...
        if inferred_sample is not None:
            self.active_simulation_sample = inferred_sample
        elif self.active_simulation_sample is None and SIMULATION_SAMPLES:
            self.active_simulation_sample = next(iter(SIMULATION_SAMPLES))

        return self.active_simulation_sample

    def _get_active_simulation_zarr_path(self) -> Optional[str]:
        if not self.is_simulation:
            return None

        camera = getattr(self.squidController, "camera", None)
        camera_zarr_path = getattr(camera, "ZARR_DATASET_PATH", None)
        if camera_zarr_path:
            return camera_zarr_path

        sample_key = self._sync_active_simulation_sample()
        if sample_key and sample_key in SIMULATION_SAMPLES:
            return SIMULATION_SAMPLES[sample_key].get("zarr_dataset_path")

        return None

    def _get_simulation_sample_info(self) -> Optional[Dict[str, Any]]:
        if not self.is_simulation:
            return None
        ...
        sample_info = dict(SIMULATION_SAMPLES.get(sample_key, {}))
        sample_info["name"] = sample_key

        active_zarr_path = self._get_active_simulation_zarr_path()
        if active_zarr_path:
            sample_info["zarr_dataset_path"] = active_zarr_path

        return sample_info

    def _build_simulation_status_payload(self) -> Dict[str, Any]:
        simulation_zarr_service_id = self._get_simulation_zarr_service_id()
        simulation_zarr_url = self.simulation_zarr_url

        if (
            simulation_zarr_url is None
            and self.is_simulation
            and self.server is not None
            and simulation_zarr_service_id is not None
        ):
            simulation_zarr_url = self._build_app_url(
                self.server, simulation_zarr_service_id
            )

        return {
            "simulation_active_sample": self._sync_active_simulation_sample(),
            "simulation_sample_info": self._get_simulation_sample_info(),
            "simulation_zarr_url": simulation_zarr_url,
            "simulation_zarr_service_id": simulation_zarr_service_id,
        }
    ...
    def _is_squid_plus_microscope(self):
        ...
        try:
            ...
            if is_squid_plus:
                logger.info(
                    "🔬 Detected Squid+ microscope - Squid+ specific endpoints will be registered"
                )
            else:
                logger.info(
                    "🔬 Detected original Squid microscope - Squid+ specific endpoints will not be registered"
                )
            ...
            logger.warning(
                f"Could not determine microscope type: {e} - assuming original Squid"
            )
            return False
    ...
    async def is_service_healthy(self, context=None):
        ...
        try:
            ...
            if result != "pong":
                raise RuntimeError(
                    f"Microscope service returned unexpected response: {result}"
                )
            ...
            if hasattr(self, "remote_server") and self.remote_server is not None:
                try:
                    ...
                    if mcp_svc is None:
                        ...
                        if mcp_result != "pong":
                            logger.warning(
                                f"MCP service returned unexpected response: {mcp_result}"
                            )
                        else:
                            logger.info("Remote server MCP service is healthy")
                except Exception as remote_error:
                    logger.warning(
                        f"Remote server health check failed: {str(remote_error)}"
                    )
    ...
    def move_by_distance(
        ...
        try:
            ...
            with self._operation_context(
                ...
                is_success, x_pos, y_pos, z_pos, x_des, y_des, z_des = (
                    self.squidController.move_by_distance_limited(x, y, z)
                )
                if is_success:
    ...
    def move_to_position(
        ...
        try:
            ...
            with self._operation_context(
                ...
                if x != 0:
                    is_success, x_pos, y_pos, z_pos, x_des = (
                        self.squidController.move_x_to_limited(x)
                    )
                    if not is_success:
                        raise Exception(
                            f"The stage cannot move to position ({x},{y},{z})mm from ({initial_x},{initial_y},{initial_z})mm because out of the limit of X axis."
                        )

                if y != 0:
                    is_success, x_pos, y_pos, z_pos, y_des = (
                        self.squidController.move_y_to_limited(y)
                    )
                    if not is_success:
                        raise Exception(
                            f"X axis moved successfully, the stage is now at ({x_pos},{y_pos},{z_pos})mm. But aimed position is out of the limit of Y axis and the stage cannot move to position ({x},{y},{z})mm."
                        )

                if z != 0:
                    is_success, x_pos, y_pos, z_pos, z_des = (
                        self.squidController.move_z_to_limited(z)
                    )
                    if not is_success:
                        raise Exception(
                            f"X and Y axis moved successfully, the stage is now at ({x_pos},{y_pos},{z_pos})mm. But aimed position is out of the limit of Z axis and the stage cannot move to position ({x},{y},{z})mm."
                        )
        ...
        try:
            ...
            current_x, current_y, current_z, current_theta = (
                self.squidController.navigationController.update_pos(
                    microcontroller=self.squidController.microcontroller
                )
            )
            ...
            simulation_status = self._build_simulation_status_payload()
            ...
            self.parameters = {
                ...
                "current_channel_name": self.channel_param_map[
                    self.squidController.current_channel
                ],
                "BF_intensity_exposure": self.BF_intensity_exposure,
                "F405_intensity_exposure": self.F405_intensity_exposure,
                "F488_intensity_exposure": self.F488_intensity_exposure,
    ...
    @schema_function(skip_self=True)
    def update_parameters_from_client(
        self,
        new_parameters: dict = Field(
            ...,
            description="Dictionary of parameters to update with key-value pairs (e.g., {'BF_intensity_exposure': [28, 20], 'dx': 0.5})",
        ),
        context=None,
    ):
        """
        ...
        try:
            ...
            with self._operation_context(
                ...
                for key, value in new_parameters.items():
                    if key in self.parameters:
                        ...
                        if hasattr(self, key):
                            ...
                            logger.warning(
                                f"Instance attribute '{key}' does not exist, parameter updated in config only"
                            )
                    else:
                        failed_params[key] = (
                            f"Parameter '{key}' not found in available parameters"
                        )
                        logger.warning(
                            f"Parameter '{key}' not found in available parameters"
                        )
                ...
                if failed_params:
                    ...
                    result["message"] += (
                        f", {len(failed_params)} parameters failed to update"
                    )
    ...
    def switch_sample(
        ...
        try:
            ...
            with self._operation_context(
                ...
                self._sync_active_simulation_sample(sample_key=sample_key)
                ...
                return {
                    ...
                    **self._build_simulation_status_payload(),
                    "message": (
    ...
    def set_wellplate_offset(
        ...
        with self._operation_context(
            ...
            try:
                if CONFIG.CACHE_CONFIG_FILE_PATH and os.path.exists(
            ...
            if config_file_path and os.path.exists(config_file_path):
                ...
                section = next(
                    (
                        s
                        for s in cfp.sections() + ["DEFAULT"]
                        if cfp.has_option(s, "wellplate_offset_x_mm")
                    ),
                    "GENERAL"
                    if "GENERAL" in cfp.sections()
                    else (cfp.sections()[0] if cfp.sections() else "DEFAULT"),
                )
                ...
                with open(config_file_path, "w") as f:
                    cfp.write(f)
                logger.info(
                    f"Updated config file with wellplate offsets: X={offset_x_mm} mm, Y={offset_y_mm} mm"
                )

            # Reload configuration
            load_config(config_file_path, False)
            return {
                "offset_x_mm": CONFIG.WELLPLATE_OFFSET_X_MM,
                "offset_y_mm": CONFIG.WELLPLATE_OFFSET_Y_MM,
            }
        ...
        try:
            async with self._async_operation_context(
                ...
                if param_name:
                    ...
                    if (
                        stored_params
                        and isinstance(stored_params, list)
                        and len(stored_params) == 2
                    ):
                        intensity, exposure_time = stored_params
                    else:
                        logger.warning(
                            f"Parameter {param_name} for channel {channel} is not properly initialized. Using defaults."
                        )
                else:
                    logger.warning(
                        f"Unknown channel {channel} in one_new_frame. Using default intensity/exposure."
                    )

                # Get the image from camera - snap_image() returns already cropped image by default
                # This is exactly what snap() does
                gray_img = await self.squidController.snap_image(
    ...
    async def get_video_frame(
        self, frame_width: int = 750, frame_height: int = 750, context=None
    ):
        """
        ...
        try:
            ...
            if self.scanning_in_progress:
                logger.debug(
                    "Scanning in progress, returning scanning placeholder frame"
                )
                placeholder = VideoFrameProcessor._create_placeholder_frame(
                    frame_width, frame_height, "Scanning in Progress..."
                )
                placeholder_compressed = VideoFrameProcessor.encode_frame_jpeg(
                ...
                return {
                    ...
                    "compression_ratio": placeholder_compressed.get(
            ...
            if self.video_idle_check_task is None or self.video_idle_check_task.done():
                self.video_idle_check_task = asyncio.create_task(
                    self._monitor_video_idle()
                )
            ...
            if frame_data is not None:
                ...
                if frame_width != buffered_width or frame_height != buffered_height:
                    ...
                    decompressed_frame = VideoFrameProcessor.decode_frame_jpeg(
                        frame_data
                    )
                    if decompressed_frame is not None:
                        ...
                        resized_compressed = VideoFrameProcessor.encode_frame_jpeg(
                            resized_frame, quality=85
                        )
                        return {
                            ...
                            "compression_ratio": resized_compressed.get(
                                "compression_ratio", 1.0
                            ),
                        ...
                        placeholder = VideoFrameProcessor._create_placeholder_frame(
                            frame_width, frame_height, "Frame decompression failed"
                        )
                        placeholder_compressed = VideoFrameProcessor.encode_frame_jpeg(
                            placeholder, quality=85
                        )
                ...
                placeholder = VideoFrameProcessor._create_placeholder_frame(
                    frame_width, frame_height, "No buffered frame available"
                )
                placeholder_compressed = VideoFrameProcessor.encode_frame_jpeg(
                    placeholder, quality=85
                )
    ...
    def configure_video_buffer(
        self, buffer_fps: int = 5, buffer_size: int = 5, context=None
    ):
        """Configure video buffering parameters for optimal streaming performance."""
        try:
            self.buffer_fps_target = max(
                1, min(30, buffer_fps)
            )  # Clamp between 1-30 FPS

            # Update buffer size
            old_size = self.frame_buffer.maxlen
            self.frame_buffer = deque(
                maxlen=max(1, min(20, buffer_size))
            )  # Clamp between 1-20 frames

            logger.info(
                f"Video buffer configured: FPS={self.buffer_fps_target}, buffer_size={self.frame_buffer.maxlen} (was {old_size})"
            )
    ...
    async def set_video_fps(self, fps: int = 5, context=None):
        ...
        try:
            ...
            if not isinstance(fps, int) or fps < 1 or fps > 30:
                raise ValueError(
                    f"Invalid FPS value: {fps}. Must be an integer between 1 and 30."
                )
    ...
    async def cleanup_for_tests(self):
        ...
        try:
            ...
            if hasattr(self, "squidController") and self.squidController:
                if (
                    hasattr(self.squidController, "camera")
                    and self.squidController.camera
                ):
    ...
    def get_video_buffering_status(self, context=None):
        ...
        try:
            ...
            frame_age = (
                self.video_buffer.get_frame_age() if self.video_buffer else float("inf")
            )

            return {
                "buffering_active": self.frame_acquisition_running,
                "buffer_size": buffer_size,
                "max_buffer_size": self.video_buffer.max_size
    ...
    def adjust_video_frame(
        self, min_val: int = 0, max_val: Optional[int] = None, context=None
    ):
        ...
        try:
            ...
            return {
                "success": True,
                "message": f"Video contrast adjusted to min={min_val}, max={max_val}.",
            }
        except Exception as e:
    ...
    async def snap(
        self,
        exposure_time: Optional[int] = Field(
            None,
            ...
            async with self._async_operation_context(
                ...
                if channel is None:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Current channel {channel_id} has no canonical name mapping"
                        )
                        channel = None
                else:
                    ...
                    try:
                        ...
                        raise Exception(
                            f"Invalid channel: {channel}. Valid channel names are: {valid_names}"
                        )
                ...
                if param_name is None:
                    raise Exception(
                        f"Invalid channel ID: {channel_id}. Valid channel IDs are: {list(self.channel_param_map.keys())}"
                    )

                # Get current intensity and exposure from channel parameters
                current_params = getattr(
                    self, param_name, [50, 100]
                )  # Default fallback
                if not (isinstance(current_params, list) and len(current_params) == 2):
                    logger.warning(
                        f"Parameter {param_name} for channel {channel_id} was not a list of two items. Using defaults."
                    )
    ...
    async def _setup_focus_map(
        self,
        focus_map_points: List[List[float]],
        do_contrast_autofocus: bool = False,
        do_reflection_af: bool = True,
    ):
        """
        ...
        logger.info(
            f"Setting up focus map with {len(focus_map_points)} reference points"
        )
        ...
        for i, point in enumerate(focus_map_points):
            if len(point) < 2:
                raise ValueError(
                    f"Each focus map point must have at least 2 values [x, y], got {len(point)}"
                )
            ...
            await asyncio.get_event_loop().run_in_executor(
                None, self.squidController.navigationController.move_to, x, y
            )
            ...
            if do_contrast_autofocus:
                ...
                logger.info(
                    f"Focus map point {i + 1}/3: Performing reflection autofocus"
                )
                await self.squidController.reflection_autofocus()
            else:
                logger.warning(
                    f"Focus map point {i + 1}/3: No autofocus method selected, using current Z"
                )
            ...
            # Add to focus map
            self.squidController.autofocusController.focus_map_coords.append(
                (x_actual, y_actual, z_actual)
            )
            logger.info(
                f"Focus map point {i + 1}/3: Added ({x_actual:.3f}, {y_actual:.3f}, {z_actual:.3f}) mm"
            )
    ...
    async def scan_plate_save_raw_images(
        self,
        well_plate_type: str = "96",
        illumination_settings: List[dict] = None,
        do_contrast_autofocus: bool = False,
        do_reflection_af: bool = True,
        wells_to_scan: List[str] = None,
        Nx: int = 3,
        Ny: int = 3,
        dx: float = 0.8,
        dy: float = 0.8,
        action_ID: str = "testPlateScan",
        focus_map_points: List[List[float]] = None,
        context=None,
    ):
        """
        ...
        """
        logger.warning(
            "DEPRECATED: scan_plate_save_raw_images is deprecated and will be removed in a future release. Use scan_start() with saved_data_type='raw_images_well_plate' instead."
        )
        ...
        try:
            ...
            if illumination_settings is None:
                logger.warning(
                    "No illumination settings provided, using default settings"
                )
            ...
            if focus_map_points is not None:
                ...
                await self._setup_focus_map(
                    focus_map_points, do_contrast_autofocus, do_reflection_af
                )
            ...
            if video_buffering_was_active:
                logger.info(
                    "Video buffering is active, stopping it temporarily during well plate scanning"
                )
                await self.stop_video_buffering()
                # Wait additional time to ensure camera fully settles after stopping video buffering
                logger.info(
                    "Waiting for camera to settle after stopping video buffering..."
                )
            ...
            logger.info(
                "Well plate scanning completed, video buffering auto-start is now re-enabled"
            )
            if operation_token is not None:
                self._release_operation(operation_token)
                self._operation_context_token.reset(reset_token)

    async def scan_flexible_positions(
        self,
        positions: List[dict] = None,
        illumination_settings: List[dict] = None,
        do_contrast_autofocus: bool = False,
        do_reflection_af: bool = True,
        action_ID: str = "flexibleScan",
        focus_map_points: List[List[float]] = None,
        move_for_autofocus: bool = False,
        context=None,
    ):
        """
        ...
        try:
            ...
            if video_buffering_was_active:
                logger.info(
                    "Video buffering is active, stopping it temporarily during flexible position scanning"
                )
                await self.stop_video_buffering()
                # Wait additional time to ensure camera fully settles after stopping video buffering
                logger.info(
            ...
            logger.info(
                f"Start flexible position scanning with {len(positions)} positions"
            )
            ...
            self.scanning_in_progress = False
            logger.info(
                "Flexible position scanning completed, video buffering auto-start is now re-enabled"
            )
            if operation_token is not None:
                self._release_operation(operation_token)
                self._operation_context_token.reset(reset_token)
    ...
    def set_illumination(
        ...
        channel: int = Field(
            0,
            description="Illumination channel: 0=Brightfield, 11=405nm, 12=488nm, 13=638nm, 14=561nm, 15=730nm",
        ),
        intensity: int = Field(
            50, description="LED illumination intensity percentage (range: 0-100)"
        ),
        context=None,
    ):
        """
        ...
        try:
            ...
            with self._operation_context(
                ...
                if self.squidController.liveController.illumination_on:
                    self.squidController.liveController.turn_off_illumination()
                    time.sleep(0.005)
                    self.squidController.liveController.set_illumination(
                        channel, intensity
                    )
                    self.squidController.liveController.turn_on_illumination()
                    time.sleep(0.005)
                ...
                if param_name:
                    current_params = getattr(
                        self, param_name, [intensity, 100]
                    )  # Default exposure if not found
                    if not (
                        isinstance(current_params, list) and len(current_params) == 2
                    ):
                        logger.warning(
                            f"Parameter {param_name} for channel {channel} was not a list of two items. Resetting with default exposure."
                        )
                        current_params = [intensity, 100]  # Default exposure
                    setattr(self, param_name, [intensity, current_params[1]])
                else:
                    logger.warning(
                        f"Unknown channel {channel} in set_illumination, parameters not updated for intensity attributes."
                    )

                logger.info(
                    f"The intensity of the channel {channel} illumination is set to {intensity}."
                )
    ...
    def set_camera_exposure(
        ...
        channel: int = Field(
            ...,
            description="Illumination channel to associate with this exposure: 0=Brightfield, 11=405nm, 12=488nm, 13=638nm, 14=561nm, 15=730nm",
        ),
        exposure_time: int = Field(
            ..., description="Camera exposure time in milliseconds (range: 1-900)"
        ),
        context=None,
    ):
        """
        ...
        try:
            ...
            with self._operation_context(
                ...
                if param_name:
                    current_params = getattr(
                        self, param_name, [50, exposure_time]
                    )  # Default intensity if not found
                    if not (
                        isinstance(current_params, list) and len(current_params) == 2
                    ):
                        logger.warning(
                            f"Parameter {param_name} for channel {channel} was not a list of two items. Resetting with default intensity."
                        )
                        current_params = [50, exposure_time]  # Default intensity
                    setattr(self, param_name, [current_params[0], exposure_time])
                else:
                    logger.warning(
                        f"Unknown channel {channel} in set_camera_exposure, parameters not updated for exposure attributes."
                    )

                logger.info(
                    f"The exposure time of the camera is set to {exposure_time}."
                )
    ...
    async def move_to_loading_position(self, context=None):
        ...
        try:
            ...
            async with self._async_operation_context(
                "move_to_loading_position", "hardware"
            ):
    ...
    @schema_function(skip_self=True)
    async def navigate_to_well(
        self,
        row: str = Field(
            "B",
            description="Well row letter (e.g., 'A', 'B', 'C', ... 'H' for 96-well)",
        ),
        col: int = Field(3, description="Well column number (e.g., 1-12 for 96-well)"),
        well_plate_type: str = Field(
            "96", description="Well plate format: '6', '12', '24', '96', or '384'"
        ),
        context=None,
    ):
        """
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        logger.warning(
                            f"Failed to fetch ICE servers, status: {response.status}"
                        )
                        return None
        except Exception as e:
    ...
    async def start_hypha_service(self, server, service_id, run_in_executor=None):
        ...
        if self.is_simulation:
            logger.info(
                "Running in simulation mode: service will be public but require context"
            )
        else:
            logger.info(
                "Running in production mode: service will be protected and require context"
            )
        ...
        if self.is_squid_plus:
            ...
            logger.info(
                f"Registered {len(squid_plus_endpoints)} Squid+ specific endpoints"
            )
        ...
        logger.info(
            f"You can also test the service via the HTTP proxy: {self.server_url}{server.config.workspace}/services/{id}"
        )
    ...
    async def start_hypha_mcp_service(self, server, service_id, run_in_executor=None):
        ...
        logger.info(
            f"You can also test the MCP service via the HTTP proxy: {self.server_url}{server.config.workspace}/services/{id}"
        )
    ...
    async def start_webrtc_service(self, server, webrtc_service_id_arg):
        ...
        async def on_init(peer_connection):
            ...
            self.metadata_data_channel = peer_connection.createDataChannel(
            ...
            async def on_connectionstatechange():
                logger.info(
                    f"WebRTC connection state changed to: {peer_connection.connectionState}"
                )
                if peer_connection.connectionState in [
                    ...
                    if self.video_track and self.video_track.running:
                        logger.info(
                            f"Connection state is {peer_connection.connectionState}. Stopping video track."
                        )
        ...
        try:
            ...
            logger.error(
                f"Failed to register WebRTC service ({self.webrtc_service_id}): {e}"
            )
            if "Service already exists" in str(e):
                logger.info(
                    f"WebRTC service {self.webrtc_service_id} already exists. Attempting to retrieve it."
                )
                try:
                    _ = await server.get_service(self.webrtc_service_id)
                    logger.info(
                        f"Successfully retrieved existing WebRTC service: {self.webrtc_service_id}"
                    )
                except Exception as get_e:
                    logger.error(
                        f"Failed to retrieve existing WebRTC service {self.webrtc_service_id}: {get_e}"
                    )
                    raise
            else:
                raise
    ...
    async def setup(self):
        ...
        remote_server = await connect_to_server(
            {
                "client_id": f"squid-remote-server-{self.service_id}-{uuid.uuid4()}",
        ...
        if not self.service_id:
            raise ValueError(
                "MICROSCOPE_SERVICE_ID is not set in the environment variables."
            )
        if self.is_local:
            ...
            server = await connect_to_server(
                {
                    "client_id": f"squid-local-server-{self.service_id}-{uuid.uuid4()}",
            ...
            server = await connect_to_server(
                {
                    "client_id": f"squid-control-server-{self.service_id}-{uuid.uuid4()}",
                    "server_url": self.server_url,
                    "token": token,
                    "workspace": "reef-imaging",
                }
        ...
        try:
            ...
            if artifact_token:
                ...
                logger.warning(
                    "REEF_WORKSPACE_TOKEN not found, artifact upload functionality disabled"
                )
        ...
        if self.artifact_manager:
            ...
            self.snapshot_manager = None
            logger.warning(
                "Snapshot manager not initialized (artifact manager unavailable)"
            )
        ...
        await self.start_hypha_mcp_service(
            self.remote_server, service_id=self.service_id
        )

        if self.is_simulation:
            await self.start_simulation_zarr_asgi_service(self.server, self.service_id)
    ...
    async def start_simulation_zarr_asgi_service(self, server, service_id):
        """Register an ASGI service that serves the active simulation Zarr dataset."""
        zarr_service_id = f"{service_id}-zarr"

        svc = await server.register_service(
            {
                "id": zarr_service_id,
                "name": "Microscope Simulation Zarr",
                "type": "asgi",
                "serve": create_simulation_zarr_asgi_app(
                    self._get_active_simulation_zarr_path,
                    logger,
                ),
                "visibility": "public",
        ...
        self.simulation_zarr_url = self._build_app_url(server, zarr_service_id)
        logger.info(
            "Simulation Zarr ASGI service registered with id: %s, accessible at %s",
            zarr_service_id,
            self.simulation_zarr_url,
        )
        return svc
    ...
    async def start_mjpeg_asgi_service(self, server, service_id):
        ...
        async def serve_mjpeg(args, context=None):
            ...
            async def stream():
                try:
                    while True:
                        frame_resp = await self.get_video_frame(
        ...
        self.mjpeg_url = self._build_app_url(server, live_view_service_id)
        logger.info(
    ...
    async def _start_video_buffering_internal(self):
        ...
        self.frame_acquisition_task = asyncio.create_task(
            self._background_frame_acquisition()
        )
    ...
    async def _stop_video_buffering_internal(self):
        ...
        if self.frame_acquisition_task:
            try:
                ...
                logger.warning(
                    "Frame acquisition task did not stop gracefully, cancelling"
                )
        ...
        while self.frame_acquisition_running:
            try:
                ...
                if consecutive_failures > 3:
                    current_fps = max(
                        1, self.buffer_fps / 2
                    )  # Halve the FPS if struggling
                    logger.warning(
                        f"Camera struggling, reducing acquisition rate to {current_fps} FPS"
                    )
                else:
                ...
                try:
                    ...
                    if self.is_simulation:
                        ...
                        raw_frame = (
                            await self.squidController.get_camera_frame_simulation(
                                channel, intensity, exposure_time
                            )
                        )
                    else:
                        ...
                        raw_frame = await asyncio.get_event_loop().run_in_executor(
                            None,
                            self.squidController.get_camera_frame,
                            channel,
                            intensity,
                            exposure_time,
                        )
                    ...
                    if raw_frame is not None:
                        frame_acquisition_time_ms = (
                            T_cam_read_complete - T_cam_start
                        ) * 1000
                        ...
                        if (
                            consecutive_failures == 0
                        ):  # Only log on successful acquisitions
                            logger.info(
                                f"LATENCY_MEASUREMENT: Background frame acquisition took {frame_acquisition_time_ms:.2f}ms, "
                        ...
                        logger.info(
                            f"LATENCY_MEASUREMENT: Background frame acquisition failed after {frame_acquisition_time_ms:.2f}ms, "
                    ...
                    if raw_frame is None:
                        ...
                        logger.warning(
                            f"Camera frame acquisition returned None - camera may be overloaded (failure #{consecutive_failures})"
                        )
                        # Create placeholder frame on None return
                        placeholder_frame = (
                            VideoFrameProcessor._create_placeholder_frame(
                                self.buffer_frame_width,
                                self.buffer_frame_height,
                                "Camera Overloaded",
                            )
                        )
                        compressed_placeholder = VideoFrameProcessor.encode_frame_jpeg(
                            placeholder_frame, quality=85
                        )

                        # Calculate gray level statistics for placeholder frame
                        placeholder_gray_stats = (
                            VideoFrameProcessor._calculate_gray_level_statistics(
                                placeholder_frame
                            )
                        )
                        ...
                        self.video_buffer.put_frame(
                            compressed_placeholder, placeholder_metadata
                        )
                        ...
                        processed_frame, gray_level_stats = (
                            VideoFrameProcessor.process_raw_frame(
                                raw_frame,
                                frame_width=self.buffer_frame_width,
                                frame_height=self.buffer_frame_height,
                                video_contrast_min=self.video_contrast_min,
                                video_contrast_max=self.video_contrast_max,
                            )
                        )
                        ...
                        compressed_frame = VideoFrameProcessor.encode_frame_jpeg(
                            processed_frame, quality=85
                        )
                        ...
                        try:
                            ...
                            current_x = (
                                self.squidController.navigationController.x_pos_mm
                            )
                            current_y = (
                                self.squidController.navigationController.y_pos_mm
                            )
                            current_z = (
                                self.squidController.navigationController.z_pos_mm
                            )
                            print(
                                f"current_x: {current_x}, current_y: {current_y}, current_z: {current_z}"
                            )
                            frame_metadata = {
                                "stage_position": {
                                    "x_mm": current_x,
                            ...
                            logger.warning(
                                f"Failed to capture stage position for metadata: {e}"
                            )
                        ...
                        processing_time_ms = (
                            T_process_complete - T_process_start
                        ) * 1000
                        compression_time_ms = (
                            T_compress_complete - T_compress_start
                        ) * 1000
                        total_time_ms = (T_compress_complete - T_cam_start) * 1000

                        # Log comprehensive performance statistics
                        logger.info(
                            f"LATENCY_PROCESSING: Background frame processing took {processing_time_ms:.2f}ms, "
                ...
                sleep_time = max(
                    0.1, (1.0 / current_fps) - elapsed
                )  # Minimum 100ms between attempts
                if sleep_time > 0:
        ...
        while self.frame_acquisition_running:
            try:
                ...
                if idle_time > self.video_idle_timeout:
                    logger.info(
                        f"Video idle for {idle_time:.1f}s (timeout: {self.video_idle_timeout}s), stopping buffering"
                    )
                    await self.stop_video_buffering()
                    break
    ...
    def get_current_well_location(
        ...
        try:
            ...
            logger.info(
                f"Current well location: {well_info['well_id']} ({well_info['position_status']})"
            )
            return well_info
        except Exception as e:
    ...
    @schema_function(skip_self=True)
    def configure_video_buffer_frame_size(
        self,
        frame_width: int = Field(750, description="Width of the video buffer frames"),
        ...
        try:
            ...
            frame_width = max(
                64, min(4096, frame_width)
            )  # Clamp between 64-4096 pixels
            frame_height = max(
                64, min(4096, frame_height)
            )  # Clamp between 64-4096 pixels
            ...
            restart_needed = (
                frame_width != old_width or frame_height != old_height
            ) and self.frame_acquisition_running

            if restart_needed:
                logger.info(
                    f"Buffer frame size changed from {old_width}x{old_height} to {frame_width}x{frame_height}, restarting buffer"
                )
            ...
            if hasattr(self, "video_track") and self.video_track:
                ...
                logger.info(
                    f"Updated WebRTC video track frame size to {frame_width}x{frame_height}"
                )

            logger.info(
                f"Video buffer frame size configured: {frame_width}x{frame_height} (was {old_width}x{old_height})"
            )
    ...
    def get_microscope_configuration(
        ...
        config_section: str = Field(
            "all",
            description="Configuration section: 'all', 'camera', 'stage', 'illumination', 'acquisition', 'limits', 'hardware', 'wellplate', 'optics', 'autofocus', or 'microscope_type'",
        ),
        include_defaults: bool = Field(
            True,
            description="Include default values from config.py (True) or only user-configured values (False)",
        ),
        context=None,
    ):
        """
        ...
        try:
            ...
            try:
                from squid_control.hardware.config import (
                    get_microscope_configuration_data,
                )
            except ImportError:
                from squid_control.hardware.config import (
                    get_microscope_configuration_data,
            ...
            logger.info(
                f"Retrieved microscope configuration for section: {config_section}"
            )
    ...
    def set_stage_velocity(
        self,
        velocity_x_mm_per_s: Optional[float] = Field(
            None,
            description="Maximum X axis velocity in mm/s (None uses config default)",
        ),
        velocity_y_mm_per_s: Optional[float] = Field(
            None,
            description="Maximum Y axis velocity in mm/s (None uses config default)",
        ),
        context=None,
    ):
        """
    ...
    @schema_function(skip_self=True)
    async def upload_zarr_dataset(
        self,
        experiment_name: str = Field(
            ...,
            description="Name of existing experiment to upload (becomes dataset name with timestamp)",
        ),
        description: str = Field(
            "", description="Optional human-readable description for the dataset"
        ),
        include_acquisition_settings: bool = Field(
            True,
            description="Include microscope settings (channels, pixel size) as dataset metadata (True recommended)",
        ),
        context=None,
    ):
        """
        ...
        try:
            ...
            async with self._async_operation_context(
                ...
                if (
                    not hasattr(self.squidController, "experiment_manager")
                    or self.squidController.experiment_manager is None
                ):
                    raise Exception(
                        "Experiment manager not initialized. Start a scanning operation first to create data."
                    )

                # Get experiment information
                experiment_info = (
                    self.squidController.experiment_manager.get_experiment_info(
                        experiment_name
                    )
                )
                ...
                if not zarr_path.exists():
                    raise Exception(
                        f"No zarr data found for experiment '{experiment_name}'. Start a scanning operation first to create data."
                    )

                total_size_mb = experiment_info.get("total_size_mb", 0)
                logger.info(
                    f"Uploading experiment '{experiment_name}' ({total_size_mb:.2f} MB)"
                )
                ...
                if include_acquisition_settings:
                    try:
                        ...
                        canvas = self.squidController.experiment_manager.get_canvas(
                            experiment_name
                        )
                        export_info = canvas.get_export_info() if canvas else {}

                        acquisition_settings = {
                            "pixel_size_xy_um": export_info.get(
                                "canvas_dimensions", {}
                            ).get("pixel_size_um", self.squidController.pixel_size_xy),
                            "channels": export_info.get(
                                "channels", ChannelMapper.get_all_human_names()
                            ),
                            "canvas_dimensions": export_info.get(
                                "canvas_dimensions", {}
                            ),
                        ...
                        logger.warning(
                            f"Could not get detailed acquisition settings: {e}"
                        )
                        acquisition_settings = {
                            "microscope_service_id": self.service_id,
                            "experiment_name": experiment_name,
                ...
                try:
                    ...
                    if "already exists" in error_str or "fileexistserror" in error_str:
                        logger.warning(
                            f"Artifact already exists, deleting and recreating..."
                        )
                        ...
                        await artifact_manager.delete(
                            artifact_id=f"{workspace}/{dataset_name}", delete_files=True
                        )
                        logger.info(
                            f"Deleted existing artifact: {workspace}/{dataset_name}"
                        )
                        server.disconnect()
                        # Now create fresh
                        await artifact.create()
                    ...
                    manifest_content = json.dumps(
                        {
                            ...
                            "description": description
                            or f"Experiment {experiment_name}",
                ...
                await artifact.commit(
                    comment=description or f"Experiment {experiment_name}"
                )

                logger.info(
                    f"Successfully uploaded experiment '{experiment_name}' as '{dataset_name}'"
                )
    ...
    async def scan_region_to_zarr(
        ...
        dx_mm: float = 0.9,
        dy_mm: float = 0.9,
        illumination_settings: Optional[List[dict]] = None,
        do_contrast_autofocus: bool = False,
        do_reflection_af: bool = False,
        action_ID: str = "normal_scan_stitching",
        timepoint: int = 0,
        experiment_name: Optional[str] = None,
        wells_to_scan: List[str] = None,
        well_plate_type: str = "96",
        well_padding_mm: float = 1.0,
        ...
        """
        logger.warning(
            "DEPRECATED: scan_region_to_zarr is deprecated and will be removed in a future release. Use scan_start() with saved_data_type='full_zarr' instead."
        )
        operation_token = None
        reset_token = None
        try:
            ...
            logger.info(
                f"Starting region scan to Zarr: {Nx}x{Ny} positions from ({start_x_mm}, {start_y_mm})"
            )
            ...
            if video_buffering_was_active:
                logger.info(
                    "Video buffering is active, stopping it temporarily during scanning"
                )
                await self.stop_video_buffering()
                # Wait additional time to ensure camera fully settles after stopping video buffering
                logger.info(
            ...
            actual_experiment_name = (
                experiment_name
                or self.squidController.experiment_manager.current_experiment_name
            )
            ...
            self.scanning_in_progress = False
            logger.info(
                "Normal scanning completed, video buffering auto-start is now re-enabled"
            )
            if operation_token is not None:
                self._release_operation(operation_token)
                self._operation_context_token.reset(reset_token)
    ...
    def reset_stitching_canvas(self, context=None):
        ...
        try:
            with self._operation_context("reset_stitching_canvas", "processing"):
                if (
                    hasattr(self.squidController, "zarr_canvas")
                    and self.squidController.zarr_canvas is not None
                ):
                ...
                return {"success": True, "message": "No stitching canvas to reset"}
        except Exception as e:
            logger.error(f"Failed to reset stitching canvas: {e}")
            raise e

    async def quick_scan_region_to_zarr(
        self,
        start_x_mm: float,
        start_y_mm: float,
        scan_width_mm: float,
        ...
        try:
            ...
            if exposure_time > 30:
                raise ValueError(
                    f"Quick scan exposure time must not exceed 30ms (got {exposure_time}ms)"
                )

            logger.info(
                f"Starting quick region scan: start=({start_x_mm}, {start_y_mm}), "
            ...
            if video_buffering_was_active:
                logger.info(
                    "Video buffering is active, stopping it temporarily during quick region scanning"
                )
                await self.stop_video_buffering()
            ...
            if uploading:
                try:
                    logger.info(
                        "Uploading experiment after quick region scan completion"
                    )
                    upload_result = await self.upload_zarr_dataset(
                        experiment_name=actual_experiment_name,
                    ...
                    logger.info(
                        "Successfully uploaded experiment after quick region scan"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to upload experiment after quick region scan: {e}"
                    )
                    # Don't raise the exception - continue with response
            ...
            return {
                ...
                "scan_parameters": {
                    "start_position": {"x_mm": start_x_mm, "y_mm": start_y_mm},
                    "scan_size": {
                        "width_mm": scan_width_mm,
                        "height_mm": scan_height_mm,
                    },
                ...
                "performance_metrics": {
                    ...
                    "scan_time_per_stripe_seconds": round(scan_duration / n_stripes, 2)
                    if n_stripes > 0
                    else 0,
                    "total_frames_acquired": total_frames,
                    "average_fps": round(total_frames / scan_duration, 2)
                    if scan_duration > 0
                    else 0,
                },
                "stitching_info": {
            ...
            self.scanning_in_progress = False
            logger.info(
                "Quick region scanning completed, video buffering auto-start is now re-enabled"
            )
            if operation_token is not None:
                self._release_operation(operation_token)
                self._operation_context_token.reset(reset_token)
    ...
    def get_stitched_region(
        ...
        center_x_mm: float = Field(
            ..., description="Region center X in stage coordinates (mm)"
        ),
        center_y_mm: float = Field(
            ..., description="Region center Y in stage coordinates (mm)"
        ),
        ...
        scale_level: int = Field(
            0,
            description="Pyramid scale level: 0=full, 1=1/4x, 2=1/16x, 3=1/64x, 4=1/256x, 5=1/1024x resolution",
        ),
        channel_name: str = Field(
            "BF LED matrix full",
            description="Channel name(s): single or comma-separated (e.g., 'BF LED matrix full' or 'BF LED matrix full,Fluorescence 488 nm Ex')",
        ),
        timepoint: int = Field(
            0, description="Time-lapse timepoint index (0 for single timepoint)"
        ),
        well_padding_mm: float = Field(
            1.0, description="Well boundary padding in millimeters"
        ),
        output_format: str = Field(
            "base64",
            description="Output format: 'base64' (PNG image) or 'array' (numpy list)",
        ),
        experiment_name: Optional[str] = Field(
            None, description="Experiment name (None uses active experiment)"
        ),
        context=None,
    ):
        """
        ...
        try:
            ...
            if isinstance(channel_name, str):
                ...
                channel_list = [
                    ch.strip() for ch in channel_name.split(",") if ch.strip()
                ]
            else:
            ...
            for i, ch_name in enumerate(channel_list):
                logger.debug(
                    f"Processing channel {i + 1}/{len(channel_list)}: '{ch_name}'"
                )
                ...
                if region is None:
                    logger.warning(
                        f"No data available for channel '{ch_name}' at ({center_x_mm:.2f}, {center_y_mm:.2f})"
                    )
                    continue

                logger.debug(
                    f"Retrieved region for channel '{ch_name}': shape={region.shape if hasattr(region, 'shape') else 'unknown'}"
                )
                channel_regions.append((ch_name, region))

            if not channel_regions:
                logger.warning(
                    f"No data available for any channels at ({center_x_mm:.2f}, {center_y_mm:.2f}) with size ({width_mm:.2f}x{height_mm:.2f})"
                )
                return {
                    "success": False,
            ...
            if output_format == "base64":
                ...
                if merged_region.dtype != np.uint8:
                    if is_rgb:
                        ...
                        # Grayscale image
                        merged_region = (
                            (merged_region / merged_region.max() * 255).astype(np.uint8)
                            if merged_region.max() > 0
                            else merged_region.astype(np.uint8)
                        )
                ...
                logger.debug(
                    f"Base64 encoded PNG: {len(img_base64)} chars, shape={merged_region.shape}"
                )
                return {
                    "success": True,
                    "data": img_base64,
        ...
        try:
            ...
            for ch_name, region_data in channel_regions:
                ...
                if region_data.max() > 0:
                    normalized_region = (
                        region_data.astype(np.float32) / region_data.max()
                    )
                else:
    ...
    async def create_experiment(
        self,
        experiment_name: str = Field(
            ..., description="Unique name for the new experiment folder"
        ),
        context=None,
    ):
        """
    ...
    async def set_active_experiment(
        self,
        experiment_name: str = Field(
            ..., description="Name of existing experiment to make active"
        ),
        context=None,
    ):
        """
    ...
    async def remove_experiment(
        self,
        experiment_name: str = Field(
            ..., description="Name of experiment to permanently delete"
        ),
        context=None,
    ):
        """
    ...
    async def reset_experiment(
        self,
        experiment_name: str = Field(
            ..., description="Name of experiment to clear all data from"
        ),
        context=None,
    ):
        """
    ...
    async def get_experiment_info(
        self,
        experiment_name: str = Field(..., description="Name of experiment to query"),
        context=None,
    ):
        """
    ...
    @schema_function(skip_self=True)
    async def process_timelapse_offline(
        self,
        experiment_id: str = Field(
            ...,
            description="Experiment ID prefix to search for matching folders (e.g., 'test-drug' matches 'test-drug-20250822T...')",
        ),
        upload_immediately: bool = Field(
            True,
            description="Upload each run to gallery immediately after stitching completes (True recommended)",
        ),
        cleanup_temp_files: bool = Field(
            True,
            description="Delete temporary zarr files after successful upload to save disk space (True recommended)",
        ),
        use_parallel_wells: bool = Field(
            True,
            description="Process 3 wells concurrently for faster completion (True=parallel, False=sequential)",
        ),
        context=None,
    ):
        """
        ...
        try:
            ...
            async with self._async_operation_context(
                ...
                if self.artifact_manager is None:
                    raise Exception(
                        "Artifact manager not initialized. Check that REEF_WORKSPACE_TOKEN is set."
                    )

                logger.info(
                    f"Starting offline processing for experiment ID: {experiment_id}"
                )
                logger.info(
                    f"Parameters: upload_immediately={upload_immediately}, cleanup_temp_files={cleanup_temp_files}, use_parallel_wells={use_parallel_wells}"
                )
                logger.info(
                    "🧵 Running offline processing in separate thread to prevent network disconnections"
                )
                ...
                def run_offline_processing():
                    ...
                    try:
                        ...
                        processor = OfflineProcessor(
                            self.squidController, self.artifact_manager, self.service_id
                        )
                        logger.info(
                            "OfflineProcessor created successfully in worker thread"
                        )
                        ...
                        try:
                            ...
                            logger.info(
                                "Calling processor.stitch_and_upload_timelapse in worker thread..."
                            )
                            ...
                            logger.info(
                                f"Offline processing completed in worker thread: {result.get('total_datasets', 0)} datasets processed"
                            )
                            logger.info(
                                f"Processing mode: {result.get('processing_mode', 'unknown')}"
                            )
                            return result
    ...
    async def _run_scan_with_state_tracking(
        self, operation_token, scan_method, *args, **kwargs
    ):
        """
        ...
        try:
            ...
            if getattr(self.squidController, "scan_stop_requested", False):
                ...
                logger.info(
                    f"Scan stopped after cancellation request: {scan_method.__name__}"
                )
            else:
    ...
    async def scan_start(
        ...
        config: dict = Field(
            ...,
            description="Scan configuration dictionary containing all scan parameters",
        ),
        context=None,
    ):
        """
        ...
        try:
            ...
            if self.scan_state["state"] == "running":
                raise Exception(
                    "A scan is already in progress. Use scan_cancel() to stop it first."
                )
            ...
            if saved_data_type not in valid_types:
                raise ValueError(
                    f"Invalid saved_data_type '{saved_data_type}'. Must be one of: {valid_types}"
                )
            ...
            if saved_data_type == "raw_images_well_plate":
                ...
                if positions is None or len(positions) == 0:
                    raise ValueError(
                        "raw_image_flexible scan requires 'positions' parameter with at least one position"
                    )
                ...
                for idx, pos in enumerate(positions):
                    if "x" not in pos or "y" not in pos:
                        raise ValueError(
                            f"Position {idx} must have 'x' and 'y' coordinates"
                        )
                ...
                if start_x_mm is None or start_y_mm is None:
                    raise ValueError(
                        "quick_zarr scan requires 'start_x_mm' and 'start_y_mm' parameters"
                    )
                if scan_width_mm is None or scan_height_mm is None:
                    raise ValueError(
                        "quick_zarr scan requires 'scan_width_mm' and 'scan_height_mm' parameters"
                    )
            ...
            if operation_token is not None:
                self._release_operation(operation_token)
            if (
                self.scan_state["scan_task"] is None
                and self.scan_state["state"] == "running"
            ):
        ...
        try:
            ...
            if task and not task.done():
                try:
                    ...
                    logger.info(
                        "Scan cancellation is still in progress in the background"
                    )
                    return {
                        "success": True,
    ...
    async def stop_scan_and_stitching(self, context=None):
        ...
        """
        logger.warning(
            "stop_scan_and_stitching is deprecated. Use scan_cancel() instead."
        )
        ...
        try:
            ...
            if result.get("success"):
                result["message"] = (
                    f"[DEPRECATED] {result['message']} (Use scan_cancel() instead)"
                )
    ...
    async def set_filter_wheel_position(
        self,
        position: int = Field(
            ..., description="Target filter wheel position (range: 1-8)"
        ),
        context=None,
    ):
        """
        ...
        try:
            ...
            async with self._async_operation_context(
                "set_filter_wheel_position", "hardware"
            ):
                ...
                success = self.squidController.filter_wheel.set_filter_position(
                    position_value
                )
                ...
                raise Exception(
                    f"Failed to set filter wheel to position {position_value}"
                )
    ...
    async def previous_filter_position(self, context=None):
        ...
        try:
            ...
            async with self._async_operation_context(
                "previous_filter_position", "hardware"
            ):
    ...
    async def switch_objective(
        ...
        objective_name: str = Field(
            ...,
            description="Target objective identifier (e.g., '4x', '10x', '20x', '40x')",
        ),
        move_z: bool = Field(
            True,
            description="Automatically adjust Z stage to compensate for objective height difference (True recommended)",
        ),
        context=None,
    ):
        """
        ...
        try:
            ...
            async with self._async_operation_context("switch_objective", "hardware"):
                ...
                if isinstance(objective_name, dict):
                    ...
                    logger.info(
                        f"Received objective_name as: {type(objective_name)} = {objective_name}"
                    )
                ...
                position_names = (
                    self.squidController.objective_switcher.get_position_names()
                )
                ...
                if position is None:
                    ...
                    raise Exception(
                        f"Objective '{objective_name_str}' not found. Available objectives: {available_objectives}"
                    )

                # Move to the found position
                if position == 1:
                    success = (
                        self.squidController.objective_switcher.move_to_position_1(
                            move_z=move_z
                        )
                    )
                elif position == 2:
                    success = (
                        self.squidController.objective_switcher.move_to_position_2(
                            move_z=move_z
                        )
                    )
                else:
                ...
                if success:
                    logger.info(
                        f"Objective switcher switched to {objective_name_str} (position {position})"
                    )

                    # Update objective-related parameters after successful switch
                    try:
                        # Update the current objective in objectiveStore
                        self.squidController.objectiveStore.current_objective = (
                            objective_name_str
                        )
                        logger.info(
                            f"Updated objectiveStore.current_objective to: {objective_name_str}"
                        )

                        # Recalculate pixel size and related parameters
                        self.squidController.get_pixel_size()
                        logger.info(
                            f"Recalculated pixel size: {self.squidController.pixel_size_xy} µm"
                        )

                        # Log the updated parameters
                        logger.info(
                            f"Objective switch completed - New objective: {objective_name_str}, "
                            f"Pixel size: {self.squidController.pixel_size_xy} µm"
                        )

                    except Exception as param_error:
                        logger.warning(
                            f"Failed to update objective parameters: {param_error}"
                        )
                    ...
                    return {
                        ...
                        "position": position,
                        "pixel_size_xy": getattr(
                            self.squidController, "pixel_size_xy", None
                        ),
    ...
    async def get_current_objective(self, context=None):
        ...
        try:
            ...
            objective_name = (
                position_names.get(position, "Unknown") if position else "Not set"
            )
    ...
    async def set_objective_switcher_speed(
        ...
        try:
            ...
            async with self._async_operation_context(
                "set_objective_switcher_speed", "hardware"
            ):
    ...
    async def get_available_objectives(self, context=None):
        ...
        try:
            ...
            positions = (
                self.squidController.objective_switcher.get_available_positions()
            )
            position_names = (
                self.squidController.objective_switcher.get_position_names()
            )
            ...
            for pos in positions:
                ...
                available_objectives.append(
                    {"position": pos, "objective_name": objective_name}
                )
