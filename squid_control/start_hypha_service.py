...
class MicroscopeHyphaService:
    ...
    async def validate_certainty_with_gpt(
        ...
        """
        Get GPT Vision feedback on cell similarity search results to assess certainty threshold quality.
        
        This endpoint helps evaluate the certainty threshold by:
        1. Searching for similar cells using the provided UUID and certainty threshold
        2. Extracting the N least similar cells from the results
        3. Creating a composite image with reference cell and validation cells
        4. Using GPT Vision to assess similarity and provide feedback
        ...
        Returns:
            dict: {
                'success': bool,
                'gpt_feedback': str,  # GPT's similarity assessment and recommendations
                'composite_image_url': str,  # Public URL to composite validation image
                'reference_uuid': str,
                'validation_count': int,
                'search_limit': int,
                'results_count': int,  # Actual number of results found
                'current_certainty': float,
                'error': str  # Present if failed
            }
        """
        try:
            logger.info(f"🔍 Starting certainty validation for UUID {target_uuid}")
            logger.info(f"   Certainty: {certainty}, Search limit: {search_limit}, Validation count: {validation_count}")
            ...
            # Use current experiment or default
            actual_experiment_name = application_id or self.squidController.experiment_manager.current_experiment_name or 'default'
            logger.info(f"   Using experiment: {actual_experiment_name}")
            
            # Step 1: Search for similar cells using the provided UUID and certainty
            logger.info("📊 Step 1/5: Searching for similar cells...")
            from squid_control.hypha_tools.weaviate_client import search_similar_by_uuid
            ...
            logger.info(f"   Found {results_count} similar cells")
            
            # Validate we have enough results
            if results_count < validation_count:
                logger.warning(f"⚠️ Only found {results_count} results, less than validation count {validation_count}")
                if results_count == 0:
                    raise Exception("No similar cells found. Try lowering the certainty threshold.")
                validation_count = results_count  # Adjust validation count to available results
            
            # Step 2: Extract least similar cells (bottom N from results)
            logger.info(f"📸 Step 2/5: Extracting {validation_count} least similar cells...")
            validation_cells = similar_results[-validation_count:]
            
            # Step 3: Extract preview images directly from search results (already fetched!)
            logger.info("🖼️ Step 3/5: Extracting preview images from search results...")
            from squid_control.hypha_tools.weaviate_client import get_cell_preview_by_uuid
            
            # Get reference cell preview (only need to fetch this one separately)
            try:
                reference_preview = await get_cell_preview_by_uuid(
                    object_uuid=target_uuid,
                    application_id=actual_experiment_name
                )
                if reference_preview is None:
                    raise Exception("Reference cell has no preview image")
            except Exception as e:
                raise Exception(f"Failed to get reference cell preview: {e}")
            
            # Extract validation cell previews directly from search results (no additional query!)
            validation_previews = []
            for idx, cell in enumerate(validation_cells):
                try:
                    cell_uuid = getattr(cell, 'uuid', None) or getattr(cell, 'id', None)
                    
                    # Extract preview from the result object (already included in search results)
                    preview = None
                    if hasattr(cell, 'properties'):
                        preview = getattr(cell.properties, 'preview_image', None)
                    elif hasattr(cell, 'preview_image'):
                        preview = cell.preview_image
                    elif isinstance(cell, dict):
                        preview = cell.get('properties', {}).get('preview_image')
                    
                    if preview:
                        validation_previews.append(preview)
                    else:
                        logger.warning(f"Cell {idx} (UUID: {cell_uuid}) has no preview image")
                except Exception as e:
                    logger.warning(f"Failed to extract preview for cell {idx}: {e}")
            
            if len(validation_previews) == 0:
                raise Exception("No validation cell previews could be retrieved")
            
            logger.info(f"   Retrieved {len(validation_previews)} validation cell previews")
            
            # Step 4: Create composite image
            logger.info("🎨 Step 4/5: Creating composite validation image...")
            from squid_control.hypha_tools.weaviate_client import create_validation_composite_image
            ...
            composite_pil.save(png_buffer, format='PNG')
            png_bytes = png_buffer.getvalue()
            
            # Save composite using snapshot manager
            import time
            metadata = {
                'type': 'certainty_validation',
                'reference_uuid': target_uuid,
                'certainty': certainty,
                'search_limit': search_limit,
                'validation_count': len(validation_previews),
                'results_count': results_count,
                'experiment': actual_experiment_name,
                'timestamp': time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())
            }
            
            composite_url = await self.snapshot_manager.save_snapshot(
                microscope_service_id=self.service_id,
                image_bytes=png_bytes,
                metadata=metadata
            )
            
            logger.info(f"   Composite saved: {composite_url}")
            
            # Step 5: Use GPT Vision to validate similarity
            logger.info("🤖 Step 5/5: Requesting GPT Vision analysis...")
            
            gpt_query = (
                f"Compare the reference cell on the left with the {len(validation_previews)} cells on the right. "
                f"These are the LEAST similar cells from a search with certainty threshold {certainty}. "
                f"Assess: 1) How many of these {len(validation_previews)} cells are actually similar to the reference? "
                f"2) Should we increase the certainty threshold (to be more restrictive) or decrease it (to be more inclusive)? "
                f"3) Provide specific reasoning based on visual features like shape, size, and appearance."
            )
            
            gpt_feedback = await self.inspect_tool(
                images=[{
                    'http_url': composite_url,
                    'title': 'Cell Similarity Validation'
                }],
                query=gpt_query,
                context_description="Images from microscope cell segmentation similarity search validation",
                context=context
            )
            ...
            logger.error(f"Failed to validate certainty: {e}", exc_info=True)
            return {
                'success': False,
                'error': str(e),
                'reference_uuid': target_uuid,
