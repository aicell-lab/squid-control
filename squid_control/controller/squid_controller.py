...
from squid_control.hardware import core, microcontroller
...
from squid_control.controller.scanning import ScanningMixin
from squid_control.controller.acquisition import AcquisitionMixin
...
if _is_simulation_mode:
    ...
    try:
        from squid_control.hardware import serial_peripherals
...
config_dir = os.path.join(os.path.dirname(os.path.dirname(path)), 'hardware')
...
class SquidController(ScanningMixin, AcquisitionMixin):
    ...
    def __init__(self, is_simulation, config_name=None, *args, **kwargs):
        ...
        if is_simulation:
            ...
            config_path = os.path.join(os.path.dirname(os.path.dirname(path)), 'config', config_file)
            ...
            squid_plus_config_main = os.path.join(os.path.dirname(os.path.dirname(path)), 'configuration_Squid+.ini')
            hcs_config_main = os.path.join(os.path.dirname(os.path.dirname(path)), 'configuration_HCS_v2.ini')
            ...
            if os.path.exists(squid_plus_config_main):
                ...
                config_dir = os.path.join(os.path.dirname(os.path.dirname(path)), 'config')
        ...
        from squid_control.utils.illumination_calibration.calibration_manager import IlluminationCalibrationManager
        
        flat_images_dir = os.path.join(os.path.dirname(os.path.dirname(path)), 'utils', 'illumination_calibration', 'flat_images')
        cache_dir = os.path.join(os.path.dirname(os.path.dirname(path)), 'config')
    ...
    async def scan_region_to_zarr(self, start_x_mm=None, start_y_mm=None, Nx=3, Ny=3, dx_mm=0.8, dy_mm=0.8,
    ...
    def _initialize_squid_plus_hardware(self):
        ...
        try:
            ...
            if getattr(CONFIG, 'FILTER_CONTROLLER_ENABLE', False):
                from squid_control.hardware.filter_wheel import (
                    FilterWheelController,
                    FilterWheelSimulation,
                )
            ...
            if getattr(CONFIG, 'USE_XERYON', False):
                from squid_control.hardware.objective_switcher import (
                    ObjectiveSwitcherController,
                    ObjectiveSwitcherSimulation,
                )
